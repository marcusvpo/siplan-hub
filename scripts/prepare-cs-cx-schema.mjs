...
const EXPECTED_TABLES = [
  ...
  'cs_cx_requests', 'cs_cx_request_attachments', 'cs_cx_audit_logs',
  'cs_cx_contacts', 'cs_cx_appointments',
  'cs_cx_routine_categories', 'cs_cx_routine_types', 'cs_cx_routine_models',
  'cs_cx_routine_model_products', 'cs_cx_routine_model_items',
  'cs_cx_office_routines', 'cs_cx_office_routine_items', 'cs_cx_routine_history',
  'cs_cx_visits', 'cs_cx_visit_checklist_items', 'cs_cx_visit_pending_items',
  'cs_cx_visit_attachments', 'cs_cx_nps_responses', 'cs_cx_nps_history',
];
const EXPECTED_RESOURCES = [
  'menu_cs_cx', 'cs_cx_home', 'cs_cx_registros', 'cs_cx_cartorios',
  'cs_cx_contatos', 'cs_cx_agendamentos', 'cs_cx_rotinas', 'cs_cx_visitas',
  'cs_cx_nps', 'cs_cx_reports', 'cs_cx_admin',
];
...
if (staticOnly) {
  console.log(`${MIGRATIONS.length} migrations CS/CX validadas; nenhuma escrita executada.`);
  process.exit(0);
}
...
function validateMigrationFiles() {
  ...
  for (const filename of MIGRATIONS) {
    ...
    if (/\b(?:BEGIN|COMMIT|ROLLBACK|VACUUM)\s*;/i.test(sql) || /\bCREATE\s+INDEX\s+CONCURRENTLY\b/i.test(sql)) {
  ...
  for (const table of EXPECTED_TABLES) {
    ...
    if (!createTable.test(combinedSql)) fail(`Criação da tabela ausente no pacote: ${table}`);
    if (!enableRls.test(combinedSql)) fail(`Ativação de RLS ausente no pacote: ${table}`);
  }
  for (const resource of EXPECTED_RESOURCES) {
    if (!combinedSql.includes(`'${resource}'`)) fail(`Permissão ausente no pacote: ${resource}`);
  }
