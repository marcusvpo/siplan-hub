...
- **Arquitetura (fila de trabalho):** o botão faz `INSERT` em `model_generation_jobs` (status `pending`). Um **worker na VM Linux da empresa** ([vm-worker/](../../vm-worker/README.md)) puxa o job por **conexão de saída** (Realtime + polling — sem túnel nem porta aberta), roda a skill `criar-modelo-mesclado` pelo Codex em modo headless autônomo dentro de `/opt/Orion.Modelos`, e devolve o `modelo.json` para o bucket `project-files` + `project_files` + `modelos_editor_available_files`. O JSON **aparece sozinho** na coluna "Disponíveis" da mesma categoria (via Realtime).
- **Gerar em lote:** cada cabeçalho de categoria (coluna enviados) tem **"Gerar todos"** (`handleGenerateCategory`), que enfileira de uma vez os arquivos que **ainda não têm modelo** — pula os já **prontos** (`done`) e os **em andamento** (`pending`/`processing`), e re-tenta os de **erro/cancelado** (enfileiramento silencioso + 1 toast-resumo). Regerar um pronto específico: clicar no ✨ da linha.
- **Badges de status** (por arquivo, via `getLatestJobFor`, que **prioriza o job ativo**): **Na fila · Nº** (posição na fila, `Clock`) → **Gerando… Xm** (tempo decorrido, `Loader2`) → **Pronto** / **Erro** (`RotateCw` = tentar de novo) / **Cancelado** (`Ban`). Clicar no badge abre o **modal de andamento ao vivo** — feed com cada passo da IA na VM (`progress`/`progress_log` + Realtime), tempo decorrido, posição na fila e botão **Cancelar**.
- **Aviso ao concluir:** uma **transição de status** no Realtime dispara um `toast` (pronto / falhou / cancelado) — o analista não precisa ficar na tela.
- **Cancelar:** botão no modal — job `pending` é cancelado direto; job `processing` liga `cancel_requested` e o worker encerra o Codex em segundos (status `cancelled`).
...
- **Validação de origem:** antes de enfileirar, checa se o arquivo ainda existe no Storage (`sourceExists` via `storage.list`) — bloqueia jobs fadados a "Object not found".
- **Concorrência/robustez:** `claim_model_generation_job` usa `FOR UPDATE SKIP LOCKED` (um worker por job); **reaper** (`requeue_stuck_model_jobs`) + **recuperação no boot** devolvem jobs travados à fila. **Anti-clobber:** upload/remoção de JSON disponível passam por RPCs atômicos (`append_available_model` / `remove_available_model`) e o transformer **não reescreve** `modelos_editor_available_files` em saves da etapa — assim o auto-save da tela nunca apaga um modelo gerado pelo worker.
...
> **Backend/infra:** migrations `20260707120000_create_model_generation_jobs` (tabela + RLS + Realtime + RPCs), `..._140000` (Traslado/Livro), `..._160000` (progresso ao vivo + heartbeat) e `..._180000` (cancelamento + `remove_available_model`). Setup do worker, Codex CLI, systemd, watchdog e segurança em [vm-worker/README.md](../../vm-worker/README.md).
