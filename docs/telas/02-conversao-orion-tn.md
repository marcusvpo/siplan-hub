...
- **Cancelar:** botão no modal — job `pending` é cancelado direto; job `processing` liga `cancel_requested` e o worker **mata o Claude** em segundos (status `cancelled`).
...
> **Backend/infra:** migrations `20260707120000_create_model_generation_jobs` (tabela + RLS + Realtime + RPCs), `..._140000` (Traslado/Livro), `..._160000` (progresso ao vivo + heartbeat) e `..._180000` (cancelamento + `remove_available_model`). Setup do worker, auto-descoberta do binário do Claude, systemd, watchdog e segurança em [vm-worker/README.md](../../vm-worker/README.md).
