...
Ao concluir uma tarefa recorrente, `complete_my_day_task(...)` encerra a ocorrência atual e cria a próxima em uma única transação. A operação usa `SECURITY INVOKER`, mantém RLS ativa, exige `work_center:edit` e também valida `work_center:create` antes de alterar uma tarefa que precisa gerar a próxima ocorrência.

Os lembretes usam o service worker do PWA quando ele está disponível e recorrem à API local de notificações da janela nos demais casos. A verificação de horário funciona enquanto o HUB ou o PWA estiver aberto; notificações confiáveis com o aplicativo encerrado exigiriam uma infraestrutura de Web Push no servidor.
...
A rota `/meu-dia/quadro` oferece um Kanban pessoal e privado. Cada usuário pode criar múltiplos quadros, definir um deles como principal, personalizar as colunas e organizar cartões por arrastar e soltar. O gesto usa um manipulador visível para não conflitar com a abertura do cartão. No desktop, a entrada da agenda e todas as colunas permanecem em uma única faixa horizontal de largura fixa sobre uma grade visual discreta; nenhuma coluna quebra para uma segunda linha. A barra nativa fica oculta e a navegação é feita ao clicar, segurar e arrastar qualquer área livre do quadro, ou pelas setas do teclado quando o quadro estiver em foco. Cabeçalhos permanecem visíveis, colunas podem ser recolhidas e cartões podem ser criados rapidamente no fim da coluna. O controle fixo no canto inferior permite reduzir ou ampliar o conteúdo entre 50% e 130%, restaurar 100%, ajustar o quadro à tela e abrir um minimapa clicável. Busca e filtros combinam título, nota, coluna, etiqueta, prioridade, prazo e origem; enquanto estão ativos, a movimentação é pausada para preservar a ordem real. Zoom, posição horizontal, densidade, minimapa e colunas recolhidas são persistidos localmente por usuário e quadro. Após mover um cartão, a confirmação oferece **Desfazer**. Botões, links e alças dos cartões preservam suas próprias ações. No celular e no PWA, as colunas são alternadas por botões e também funcionam como alvos de movimentação por toque, mantendo o conteúdo dentro da viewport sem rolagem horizontal.

Os cartões aceitam notas, prioridade, até oito etiquetas, prazo opcional, checklist e um link interno do HUB. Cartões com prazo são incorporados à **Minha agenda** e à fila de prioridades sem serem duplicados em `my_day_tasks`. A conclusão é uma propriedade explícita da coluna e pode ser ativada em **Configurar**, portanto continua correta mesmo quando a coluna recebe um nome personalizado.

A coluna inteligente **Entrada da agenda** reúne automaticamente tarefas pessoais pendentes e compromissos atribuídos ao usuário em CS/CX e Implantação, dentro da mesma janela operacional da Central de Trabalho. O usuário pode filtrar por origem, arrastar o item para a coluna desejada ou usar **Adicionar**. A conversão grava no link interno uma referência estável à origem; isso retira o item da entrada e impede que o cartão gere uma segunda ocorrência na agenda. Depois da conversão, título, prazo e prioridade acompanham automaticamente a origem; notas, etiquetas, checklist, coluna e conclusão permanecem sob controle do quadro. Itens arquivados continuam reconhecidos para não reaparecerem involuntariamente; ao excluir definitivamente o cartão, o compromisso volta a ficar disponível enquanto permanecer ativo na origem.

Os dados são armazenados em `public.my_day_boards`, `public.my_day_board_columns` e `public.my_day_board_cards`. As três tabelas usam RLS por `user_id = auth.uid()` e o recurso `work_board`; o usuário não consegue consultar nem relacionar dados de outro proprietário. Alterações de conclusão, reordenação de colunas e sincronização de cartões usam funções transacionais para não deixar o quadro parcialmente atualizado.
