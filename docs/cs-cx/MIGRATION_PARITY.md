...
| Rotinas | aplicações, configuração, análise, histórico, modelos, itens, categorias, tipos, reordenação, relatórios e PDF | em migração; aplicações, análise e administração implementadas; PDF individual e painel consolidado com filtros, gráficos, PDF e Excel implementados; histórico detalhado pendente |
