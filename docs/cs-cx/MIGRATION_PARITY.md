...
| NPS | respostas, importação, classificação, estatísticas, histórico, reatribuição, webhook, exclusão e PDF | em migração; respostas, classificação, indicadores, histórico, reatribuição por edição, importação CSV/XLSX idempotente e PDF implementados; webhook pendente |
