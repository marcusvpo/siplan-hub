...
| Cartórios | cadastro, edição, ativação, produtos, período de implantação, registros, histórico e PDFs | em migração; cadastro, CRUD, filtros, visualização somente leitura, produtos e múltiplos responsáveis por produto implementados |
| Contatos | lista, filtros, criação, edição, exclusão, produtos, estatísticas e PDFs | em migração; lista, CRUD, filtros por responsável/período, múltiplos produtos, indicadores e PDF implementados |
| Agendamentos | lista, calendário, criação, edição, realização, conclusão, cancelamento, remarcação, impressão e PDF | em migração; lista, calendário, CRUD, filtros por responsável/período, PDF, mudanças de estado e consulta rápida somente leitura das observações implementados |
| Rotinas | aplicações, configuração, análise, histórico, modelos, itens, categorias, tipos, reordenação, relatórios e PDF | em migração; fluxo funcional implementado, incluindo histórico detalhado com filtros e contexto preservado; carga comparativa e homologação pendentes |
| Visitas | criação, edição, checklist, pendências, anexos, geração de solicitações, status e PDF | em migração; lista, CRUD, filtro por período, status, checklist, pendências, anexos nativos, geração transacional de solicitações e PDF implementados; cópia dos binários legados concluída e homologação humana pendente |
| NPS | respostas, importação, classificação, estatísticas, histórico, reatribuição, questionários, links públicos, webhook, exclusão e PDF | em migração; fluxo legado preservado e formulário nativo implementado com editor de questionários, convite individual de uso único, validade, cancelamento, snapshot, resposta sem login, gravação automática, BI, indicadores de cartórios avaliados/não avaliados e atalhos por classificação; relatório PDF corrigido para listar clientes com notas 9 e 10 do recorte e evitar a apresentação enganosa de NPS 0 por cartório; homologação humana pendente |
