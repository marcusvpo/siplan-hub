CREATE OR REPLACE FUNCTION public.cs_cx_set_routine_items_bulk_v3(
  ...
  p_active BOOLEAN,
  p_routine_model_ids UUID[] DEFAULT NULL,
...
BEGIN
  ...
  WITH target_items AS MATERIALIZED (
    ...
    WHERE routine.registry_office_id = p_registry_office_id
      AND (
        p_routine_model_ids IS NULL
        OR routine.routine_model_id = ANY(p_routine_model_ids)
      )
...
REVOKE ALL ON FUNCTION public.cs_cx_set_routine_items_bulk_v3(
  UUID, BOOLEAN, UUID[], TEXT, TEXT, TIMESTAMPTZ
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.cs_cx_set_routine_items_bulk_v3(
  UUID, BOOLEAN, UUID[], TEXT, TEXT, TIMESTAMPTZ
) TO authenticated;

COMMENT ON FUNCTION public.cs_cx_set_routine_items_bulk_v3(
  UUID, BOOLEAN, UUID[], TEXT, TEXT, TIMESTAMPTZ
) IS 'Atualiza em massa os itens das rotinas de um cartorio, com filtro opcional por modelos de produto.';
