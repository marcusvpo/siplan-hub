...
export function ModelosEditorWorkspace({ project, onUpdate }: ModelosEditorWorkspaceProps) {
    ...
    const renderFileRow = (file: AttachedFile, type: 'sent' | 'available', list: AttachedFile[]) => (
        <div key={file.id} className={cn("flex items-center justify-between p-2.5 rounded-md bg-white dark:bg-slate-900 border border-border/50 dark:border-slate-800 text-sm transition-all duration-200 shadow-sm", file.isDone && "bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/50")}>
            <div className="flex items-center gap-3 overflow-hidden">
                <Checkbox
                    checked={!!file.isDone}
                    disabled={!canEditProjects}
                    onCheckedChange={() => handleToggleFileDone(file, type, list)}
                    className="data-[state=checked]:bg-emerald-500 data-[state=checked]:border-emerald-500 rounded flex-shrink-0 h-4 w-4"
                />
            ...
            <div className="flex items-center gap-1 shrink-0">
                <Button variant="ghost" size="icon" className="h-7.5 w-7.5 text-indigo-500 hover:text-indigo-600 dark:text-indigo-400 dark:hover:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/20" title="Visualizar arquivo" onClick={(e) => { e.preventDefault(); handleFileView(file); }}>
                    <Eye className="h-3.5 w-3.5" />
                </Button>
                <Button variant="ghost" size="icon" className="h-7.5 w-7.5 transition-colors hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20" title="Baixar" onClick={(e) => { e.preventDefault(); handleFileDownload(file); }}>
                    <Download className="h-3.5 w-3.5" />
                </Button>
                {canDeleteFiles && (
                <Button variant="ghost" size="icon" className="h-7.5 w-7.5 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20" title="Excluir" onClick={(e) => { e.preventDefault(); handleRemoveFile(file, type, list); }}>
    ...
    return (
        <div className="col-span-full w-full flex-1 flex flex-col min-h-0 space-y-4">
            {renderProgress()}

            {/* Force 2 columns always */}
            <div className="flex-1 min-h-0 grid grid-cols-2 gap-4 w-full">
                {/* Modelos Enviados */}
                <div className="flex flex-col min-h-0 p-4 rounded-xl border border-indigo-100 dark:border-indigo-900/30 bg-indigo-50/30 dark:bg-indigo-900/10 space-y-3">
                    ...
                    {stage.sentFiles && stage.sentFiles.length > 0 && (
                        <div className="flex-1 overflow-y-auto pr-1.5 space-y-2 scrollbar-thin scrollbar-thumb-indigo-200 scrollbar-track-transparent hover:scrollbar-thumb-indigo-300 transition-colors">
                ...
                {/* Modelos Disponíveis */}
                <div className="flex flex-col min-h-0 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/30 bg-emerald-50/30 dark:bg-emerald-900/10 space-y-3">
                    <div className="flex items-center justify-between shrink-0">
                    ...
                    {stage.availableFiles && stage.availableFiles.length > 0 && (
                        <div className="flex-1 overflow-y-auto pr-1.5 space-y-2 scrollbar-thin scrollbar-thumb-emerald-200 scrollbar-track-transparent hover:scrollbar-thumb-emerald-300 transition-colors">
