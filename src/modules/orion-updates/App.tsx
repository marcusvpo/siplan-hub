...
import { adminApi, Item, Product, Publication, Version, System, publicApi, type PublicSettings } from "./api";
import { PublicMaintenance } from "./PublicMaintenance";
...
function SharedPostPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    publicApi.settings()
      ...
      .catch(() => undefined)
      .finally(() => { if (!cancelled) setCheckingSettings(false); });
    return () => { cancelled = true; };
  }, []);

  useEffect(() => {
    if (checkingSettings || (settings && !settings.public_enabled)) return;
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    document.title = `${post.titulo} — Orion Changelog`;
    ...
    <header className="topbar">
      <div className="container topbar-row reader-topbar-row">
        ...
        <div className="reader-header-center">
          <img src={appPath("/assets/Siplan_logo.png")} alt="Logo Siplan" className="reader-header-logo" />
        </div>
...
function AdminApp() {
  ...
  if (!access) return <main className="management-access-state"><h1>Não foi possível verificar seu acesso</h1><p>A Gestão permanece protegida. Tente novamente em instantes.</p><div><button className="secondary" onClick={() => void refresh()}>Tentar novamente</button><AppLink href={HOME_PATH}>Voltar ao blog</AppLink></div></main>;
  if (!access.canManage || !access.admin) {
    return <main className="management-access-state"><h1>Acesso restrito</h1><p>Seu perfil não possui a permissão de Gestão da Central de Atualizações.</p><div><button className="secondary" onClick={() => void refresh()}>Verificar acesso novamente</button><AppLink href={HOME_PATH}>Voltar ao blog</AppLink></div></main>;
  }
  return <ManagementApp key={access.admin.id} admin={access.admin} />;
}
...
function ManagementApp({ admin }: { admin: { id: string; email: string } }) {
  ...
  async function refresh() {
    try {
      const [postResult, versionResult, settingsResult] = await Promise.all([
        adminApi.list(),
        adminApi.versions(),
      ...
      if (settingsResult) {
        ...
        setMaintenanceTitleInput(settingsResult.maintenance_title);
        setMaintenanceMessageInput(settingsResult.maintenance_message);
      }
  ...
  async function togglePublicLink() {
    ...
    const nextState = !settings.public_enabled;
    setUpdatingSettings(true);
    try {
      const updated = await adminApi.updateSettings({
        public_enabled: nextState,
        maintenance_title: settings.maintenance_title,
        maintenance_message: settings.maintenance_message,
      });
      setSettings(updated);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Erro ao alterar status do link público.");
    } finally {
      setUpdatingSettings(false);
    }
  }

  async function saveMaintenanceSettings(e: React.FormEvent) {
    e.preventDefault();
    if (!settings || updatingSettings) return;
    setUpdatingSettings(true);
    try {
      const updated = await adminApi.updateSettings({
        ...
        maintenance_title: maintenanceTitleInput,
        maintenance_message: maintenanceMessageInput,
      });
      setSettings(updated);
      setShowSettingsModal(false);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Erro ao salvar mensagem de manutenção.");
    } finally {
      setUpdatingSettings(false);
    }
  }
  ...
  async function saveVersion(codigo: string, sistema: System) {
    if (!access?.canCreate) throw new Error("Seu perfil não permite criar versões.");
    await adminApi.createVersion({ codigo, sistema }); setCreatingVersion(false); setError(""); await refresh();
  }

  async function edit(id: number) {
    if (!access?.canEdit) { setError("Seu perfil não permite editar publicações."); return; }
  ...
  async function save(draft: EditorDraft) {
    if (editing === "new") {
      if (!access?.canCreate) throw new Error("Seu perfil não permite criar publicações.");
      await adminApi.create(draft);
    } else if (editing) {
      if (!access?.canEdit) throw new Error("Seu perfil não permite editar publicações.");
      await adminApi.update(editing.id, draft);
    }
    setEditing(null); setError(""); await refresh();
  }

  async function removePost(id: number) {
    if (!access?.canDelete) { setError("Seu perfil não permite excluir publicações."); return; }
  ...
  async function publish(id: number) {
    if (!access?.canEdit) { setError("Seu perfil não permite publicar conteúdos."); return; }
  ...
  return <div className="page management-page">
    <header className="topbar"><div className="container topbar-row">
      <div className="brand-title"><img src={appPath("/assets/Siplan_logo.png")} alt="Logo Siplan" /><div><h1>Orion Changelog</h1><p><span className="management-mode">Modo Gestão</span><span className="management-email">{admin.email}</span></p></div></div>
    ...
    <main className="container management-main">
      <div className="public-link-toggle-card flex flex-wrap items-center justify-between gap-4 p-4 mb-6 rounded-xl border border-border/70 bg-card shadow-sm text-foreground">
        <div className="flex items-center gap-3">
          <div className={`h-3 w-3 rounded-full shrink-0 ${settings?.public_enabled ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`} />
          <div>
            <div className="flex items-center gap-2 text-sm font-semibold">
              <span>Link Público (/atualizacoes/inicio):</span>
              <span className={settings?.public_enabled ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-rose-600 dark:text-rose-400 font-bold"}>
                {settings?.public_enabled ? "Ativo 🟢" : "Desativado (Em Manutenção) 🔴"}
              </span>
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
              {settings?.public_enabled
                ? "Visitantes sem login acessam o blog normalmente."
                : "Visitantes visualizam a tela de manutenção sem ter acesso ao conteúdo."}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => {
              if (settings) {
                setMaintenanceTitleInput(settings.maintenance_title);
                setMaintenanceMessageInput(settings.maintenance_message);
                setShowSettingsModal(true);
              }
            }}
          >
            ⚙️ Mensagem de Manutenção
          </button>
          {access?.canEdit && (
            <button
              type="button"
              className={settings?.public_enabled ? "danger" : "primary"}
              disabled={updatingSettings || !settings}
              onClick={() => void togglePublicLink()}
            >
              {updatingSettings
                ? "Atualizando..."
                : settings?.public_enabled
      ...
      {showSettingsModal && settings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl bg-card border border-border p-6 shadow-2xl text-foreground">
            <h3 className="text-lg font-bold mb-1">Personalizar Modo de Manutenção</h3>
            <p className="text-xs text-muted-foreground mb-4">
              Defina o título e a mensagem apresentados aos visitantes quando o link público estiver desativado.
            </p>
            <form onSubmit={(e) => void saveMaintenanceSettings(e)} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold mb-1 text-foreground">Título do Alerta</label>
                <input
                  type="text"
                  className="w-full"
              ...
              <div>
                <label className="block text-xs font-semibold mb-1 text-foreground">Mensagem Explicativa</label>
                <textarea
                  className="w-full text-sm"
                  rows={4}
                  value={maintenanceMessageInput}
                  onChange={(e) => setMaintenanceMessageInput(e.target.value)}
                  placeholder="Ex: O Orion Changelog está passando por melhorias..."
                  required
                />
              </div>
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" className="primary" disabled={updatingSettings}>
                  {updatingSettings ? "Salvando..." : "Salvar Configuração"}
                </button>
              </div>
            </form>
          </div>
