...
import { getOrionUpdateMediaId, getOrionUpdateMediaUrl } from '@/hooks/useOrionUpdatesData';
...
export function safePostHtml(html: string, _preview = false, forStorage = false) {
  ...
  fragment.querySelectorAll('img').forEach(img => {
    ...
    const imageId = getOrionUpdateMediaId(source);
    if (!imageId) img.remove();
    else {
      img.setAttribute(
        'src',
        forStorage ? `/orion-update-media/${imageId}.webp` : getOrionUpdateMediaUrl(imageId),
      );
    }
  });
