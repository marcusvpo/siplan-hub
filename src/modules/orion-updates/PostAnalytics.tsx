...
export function PostAnalytics({ product, type, onEdit, canEdit = false }: { product: string; type: string; onEdit: (id: number) => void; canEdit?: boolean }) {
  ...
  return <section className="analytics-panel manager-dashboard" aria-labelledby="analytics-heading">
    ...
    {summary && <>
        ...
        {result && (result.data.length === 0 ? <div className="manager-state"><Icon name="document"/><h4>Nenhuma publicação encontrada para estes filtros.</h4><p>{focus === 'feedback' ? 'Não há reações negativas neste recorte.' : 'Experimente outro sistema, tipo, status ou termo de busca.'}</p>{(focus !== 'todos' || search || status !== 'todos') && <button type="button" className="secondary" onClick={clearLocalFilters}>Ampliar análise</button>}</div> : <>
          <div className="manager-table-wrap"><table className="manager-table"><caption className="manager-sr-only">Métricas acumuladas por publicação. A aprovação considera apenas as avaliações recebidas.</caption>
            ...
            <tbody>{result.data.map(post => <tr key={post.id} className={post.dislikes > 0 ? 'manager-has-feedback' : ''}>
              ...
              <td className="manager-row-actions"><button className="manager-reasons-button" type="button" disabled={post.dislikes === 0} onClick={() => setSelected(post)} aria-label={`Ver motivos de Não gostei: ${post.titulo}`}><Icon name="message"/>Ver motivos</button>{canEdit && <button className="manager-edit-button" type="button" onClick={() => onEdit(post.id)} aria-label={`Revisar publicação: ${post.titulo}`}><Icon name="edit"/>Revisar</button>}</td>
            </tr>)}</tbody>
          </table></div>
      ...
      <details className="manager-method"><summary>Como interpretar os indicadores</summary><div><p><strong>Visualizações:</strong> cada usuário autenticado conta uma vez por publicação. O total soma essas leituras; o mesmo usuário pode aparecer em mais de uma publicação. O número de leitores no conjunto é deduplicado entre elas.</p><p><strong>Aprovação:</strong> Gostei ÷ (Gostei + Não gostei). Sem reações, mostramos “Sem avaliações”, não 0%. Considere a quantidade de avaliações antes de tirar conclusões. Cada usuário mantém uma reação por publicação.</p><p><strong>Compartilhamentos:</strong> ações concluídas de copiar o link ou usar o compartilhamento do dispositivo, não envios ou recebimentos confirmados em outros aplicativos.</p><p>Os números são acumulados, sem comparação temporal. Rascunhos, agendamentos futuros, arquivados e sistemas inativos não entram nos recortes “Sem visualizações” e “Lidas, sem avaliações”. Os motivos correspondem aos votos negativos atuais, sem controle de revisão concluída.</p></div></details>
    {selected && <DislikeReasons key={`${selected.id}-${revision}`} post={selected} canEdit={canEdit} onClose={() => setSelected(null)} onEdit={() => onEdit(selected.id)} />}
  </section>;
}
...
function DislikeReasons({ post, onClose, onEdit, canEdit }: { post: PostMetrics; onClose: () => void; onEdit: () => void; canEdit: boolean }) {
  ...
  return createPortal(<div className="orion-blog-module"><dialog ref={dialog} className="analytics-feedback-dialog" aria-labelledby="analytics-feedback-title" onCancel={event => { event.preventDefault(); onClose(); }} onKeyDown={event => {
    ...
    {canEdit && <button className="primary manager-review" type="button" onClick={() => { onClose(); onEdit(); }}><Icon name="edit"/>Revisar publicação</button>}
  </section></dialog></div>, document.body);
}
