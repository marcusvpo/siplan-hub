        ...
        <div className="flex gap-6 h-full min-w-max pb-4">
          {COLUMNS.map(column => (
            <div key={column.id} className="w-80 flex flex-col gap-4">
              ...
              <div className="flex-1 overflow-y-auto space-y-3 bg-muted/20 p-2 rounded-xl border border-dashed border-muted-foreground/20">
                {groupedProjects[column.id].map(project => (
                  <Card 
                    key={project.id}
                    className="cursor-pointer hover:ring-2 hover:ring-primary/50 transition-all border-none shadow-md group"
