...
export default function EditarChecklistComercial() {
  ...
  // Query templates
  const { data: templates = [], isLoading: isLoadingTemplates } = useFormTemplates("commercial_checklist", selectedSystem);
  const { data: activeTemplate, isLoading: isLoadingActive } = useActiveTemplate("commercial_checklist", selectedSystem);
  ...
  const currentSchema = React.useMemo(() => {
    return convertVisualToJSONSchema(
      questions,
      `Checklist Estrutural da Serventia (${selectedSystem})`,
      "Coleta de informações básicas sobre estrutura, pessoas e setores"
    );
  ...
  const handlePublish = async () => {
    ...
    try {
      await publishMutation.mutateAsync({
        ...
        notes: notes || `Checklist comercial atualizado para ${selectedSystem}`,
      });

      toast({
        title: "Sucesso!",
        description: "Checklist Comercial publicado e ativado com sucesso.",
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-black tracking-tight text-foreground bg-gradient-to-r from-primary to-orange-500 bg-clip-text text-transparent">
              Criador de Checklist Comercial
            </h1>
          </div>
          <p className="text-sm text-muted-foreground pl-10">
            Customize de forma visual as perguntas do checklist enviadas para os clientes do Comercial.
          </p>
        </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription className="text-xs mt-1">
                    Crie e reordene os campos que o cliente precisará responder no link público.
                  </CardDescription>
                </div>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Ex: Adicionado nova pergunta sobre servidores locais e largura de banda da internet."
          ...
          <Card className="shadow-lg border-muted/50 bg-card flex-1 flex flex-col">
            <CardHeader className="bg-primary/5 pb-3 border-b border-primary/10">
              ...
              <CardDescription className="text-xs">
                Veja como o formulário ficará para o cliente preencher.
              </CardDescription>
            </CardHeader>
