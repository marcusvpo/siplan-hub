...
const emptyForm: CsCxVisitInput = {
  registry_office_id: "",
  visitor_profile_id: "",
...
export default function CsCxVisits() {
  ...
  const { canCreate, canEditRecord, canDeleteRecord } =
    useCsCxRecordPermissions("cs_cx_visitas");
  ...
  const openCount = visits.filter(
    (visit) => visit.status !== "concluido",
  ).length;
  const pendingCount = visits
    .flatMap((visit) => visit.pending_items)
    .filter((item) => item.status !== "resolvida").length;
  const checklistCount = visits
    .flatMap((visit) => visit.checklist)
  ...
  function openEdit(visit: CsCxVisit) {
    setForm({
      id: visit.id,
      registry_office_id: visit.registry_office_id,
      visitor_profile_id: visit.visitor_profile_id ?? "",
      visit_date: visit.visit_date,
      start_time: visit.start_time?.slice(0, 5) ?? "",
  ...
  async function handleSave() {
    try {
      await saveVisit.mutateAsync(form);
      setDialogOpen(false);
      toast({ title: form.id ? "Visita atualizada" : "Visita criada" });
    } catch (mutationError) {
      toast({
  ...
  async function handleStatus(id: string, status: string) {
    try {
      await setVisitStatus.mutateAsync({ id, status });
    ...
    <div className="container mx-auto max-w-[1600px] space-y-4 px-4 py-4 lg:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Planejamento, execução, checklist e pendências das visitas aos
              cartórios
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <Metric
          ...
          value={visits.length}
        />
        <Metric icon={MapPin} label="Em acompanhamento" value={openCount} />
        <Metric
          icon={ClipboardList}
        ...
        <CardContent className="grid gap-2 p-3 lg:grid-cols-[minmax(260px,1fr)_200px_155px_155px]">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Buscar cartório, objetivo ou visitante..."
            />
          </div>
          <Select value={statusFilter} onValueChange={updateStatusFilter}>
            ...
            <SelectContent>
              ...
              {CS_CX_VISIT_STATUSES.map((status) => (
          ...
          <Input
            className="h-9"
            aria-label="Data inicial da visita"
            type="date"
            value={dateFrom}
          ...
          <Input
            className="h-9"
            aria-label="Data final da visita"
            type="date"
            value={dateTo}
      ...
      <div className="space-y-2">
        {pagedVisits.map((visit) => {
          ...
          const canEdit = canEditRecord(visit.created_by);
          const canDelete = canDeleteRecord(visit.created_by);
          return (
            <Card key={visit.id}>
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <CardTitle
                        ...
                        title={
                          visit.registry_office?.name ?? "Cartório removido"
                        }
                      >
                        {visit.registry_office?.name ?? "Cartório removido"}
                      </CardTitle>
                      <Badge
                        variant={
                          visit.status === "concluido" ? "default" : "secondary"
                        }
                    ...
                    <CardDescription className="mt-0.5 text-xs">
                      ...
                      {visit.start_time
                        ...
                        : ""}{" "}
                      · {visit.visitor?.full_name ?? "Sem visitante"}
                    </CardDescription>
                    <p
                      className="mt-1 max-w-3xl truncate text-xs"
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Registre o planejamento e o responsável pela visita.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Visitante">
              <Select
                value={form.visitor_profile_id || "current"}
                onValueChange={(value) =>
                  setForm((current) => ({
                    ...current,
                    visitor_profile_id: value === "current" ? "" : value,
                  }))
                }
              >
                <SelectTrigger>
            ...
            <div className="md:col-span-2">
              <Label htmlFor="visit-objective">Objetivo</Label>
              <Textarea
                id="visit-objective"
            ...
            <div className="md:col-span-2">
              <Label htmlFor="visit-notes">Observações gerais</Label>
              <Textarea
                id="visit-notes"
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Checklist, pendências e anexos vinculados também serão removidos
              do HUB.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function VisitPaginationBar({
  ...
  return (
    <div className="flex flex-col gap-2 border-t pt-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
      <span
        aria-label={`Mostrando ${firstItem} a ${lastItem} de ${totalItems} visitas`}
      >
        Mostrando{" "}
