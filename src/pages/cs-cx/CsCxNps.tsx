...
import { FileDown, Frown, Meh, Pencil, Plus, RefreshCw, Search, Smile, Star, Trash2, TriangleAlert, Upload } from "lucide-react";
...
export default function CsCxNps() {
  ...
  return <div className="container mx-auto max-w-7xl space-y-6 p-6">
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><div className="flex items-center gap-2 text-sm font-medium text-rose-600 dark:text-rose-300"><Star className="h-4 w-4" />CS/CX</div><h1 className="mt-1 text-3xl font-black tracking-tight">NPS</h1><p className="text-sm text-muted-foreground">Satisfação, classificação e evolução das avaliações dos cartórios.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={!filtered.length || isExporting} onClick={handleExport}><FileDown className="mr-2 h-4 w-4" />Exportar PDF</Button>{canCreate && <><Button variant="outline" onClick={() => setImportOpen(true)}><Upload className="mr-2 h-4 w-4" />Importar arquivo</Button><Button onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Nova resposta</Button></>}</div></div>
