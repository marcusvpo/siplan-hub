...
import { ChevronLeft, ChevronRight, FileDown, Frown, Meh, Pencil, Plus, RefreshCw, Search, Smile, Star, Trash2, TriangleAlert, Upload } from "lucide-react";
...
export default function CsCxNps() {
  const { responses, history, isLoading, error, refetch, saveResponse, deleteResponse, importResponses } = useCsCxNps();
  ...
  const responseTotalPages = Math.max(1, Math.ceil(filtered.length / responsePageSize));
  const currentResponsePage = Math.min(responsePage, responseTotalPages);
  const pagedResponses = useMemo(
    () => filtered.slice((currentResponsePage - 1) * responsePageSize, currentResponsePage * responsePageSize),
  ...
  const pagedHistory = useMemo(
    () => history.slice((currentHistoryPage - 1) * historyPageSize, currentHistoryPage * historyPageSize),
    [currentHistoryPage, history, historyPageSize],
  );
  const updateSearch = (value: string) => { setSearch(value); setResponsePage(1); };
  const updateClassification = (value: string) => { setClassification(value); setResponsePage(1); };
  const updateResponsePageSize = (value: string) => { setResponsePageSize(Number(value)); setResponsePage(1); };
  ...
  async function handleImport() {
    ...
    try {
      const parsed = importFile.name.toLocaleLowerCase("pt-BR").endsWith(".xlsx")
        ? await parseNpsXlsx(await importFile.arrayBuffer(), office.name)
        : parseNpsCsv(await importFile.text(), office.name);
      if (!parsed.rows.length) throw new Error(parsed.errors[0] ?? "Nenhuma resposta válida encontrada.");
      const result = await importResponses.mutateAsync({ registryOfficeId: office.id, rows: parsed.rows });
      setImportOpen(false);
      setImportFile(null);
      toast({ title: "Importação concluída", description: `${result.imported} importadas, ${result.duplicates} duplicadas e ${parsed.errors.length} linhas inválidas.` });
    } catch (mutationError) { toast({ title: "Não foi possível importar", description: messageOf(mutationError), variant: "destructive" }); }
  }
  async function handleExport() {
    setIsExporting(true);
    try {
      const filters = [classification === "all" ? "Todas as classificações" : `Classificação: ${CLASS_LABELS[classification] ?? classification}`, search.trim() ? `Busca: ${search.trim()}` : "Sem filtro de busca"];
  ...
  return <div className="container mx-auto max-w-[1600px] space-y-4 px-4 py-4 lg:px-6">
    <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-300"><Star className="h-4 w-4" /></span><div><h1 className="text-2xl font-black leading-none tracking-tight">NPS</h1><p className="mt-1 text-xs text-muted-foreground">Satisfação, classificação e evolução das avaliações dos cartórios</p></div></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={!filtered.length || isExporting} onClick={handleExport}><FileDown className="mr-2 h-4 w-4" />Exportar PDF</Button>{canCreate && <><Button size="sm" variant="outline" onClick={() => setImportOpen(true)}><Upload className="mr-2 h-4 w-4" />Importar arquivo</Button><Button size="sm" onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Nova resposta</Button></>}</div></div>
    ...
    <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><Metric icon={Star} label="NPS geral" value={nps} /><Metric icon={Smile} label="Promotores" value={promoters} /><Metric icon={Meh} label="Neutros" value={neutrals} /><Metric icon={Frown} label="Detratores" value={detractors} /></div>
    <Tabs defaultValue="responses"><TabsList className="h-9"><TabsTrigger className="h-7" value="responses">Respostas</TabsTrigger><TabsTrigger className="h-7" value="history">Histórico</TabsTrigger></TabsList><TabsContent value="responses" className="mt-3 space-y-3"><Card><CardContent className="grid gap-2 p-3 md:grid-cols-[minmax(260px,1fr)_220px]"><div className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="h-9 pl-9" value={search} onChange={(event) => updateSearch(event.target.value)} placeholder="Buscar respondente ou cartório..." /></div><Select value={classification} onValueChange={updateClassification}><SelectTrigger className="h-9"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas as classificações</SelectItem><SelectItem value="PROMOTOR">Promotores</SelectItem><SelectItem value="NEUTRO">Neutros</SelectItem><SelectItem value="DETRATOR">Detratores</SelectItem></SelectContent></Select></CardContent></Card><Card><CardContent className="p-0"><Table className="[&_td]:px-3 [&_td]:py-2 [&_th]:h-9 [&_th]:px-3"><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Cartório</TableHead><TableHead>Respondente</TableHead><TableHead>Nota</TableHead><TableHead>Classificação</TableHead><TableHead>Motivo</TableHead><TableHead className="w-20" /></TableRow></TableHeader><TableBody>{pagedResponses.map((response) => <TableRow key={response.id}><TableCell className="whitespace-nowrap text-xs">{formatDate(response.responded_at)}</TableCell><TableCell className="max-w-56 truncate font-medium" title={response.registry_office?.name ?? response.respondent_office}>{response.registry_office?.name ?? response.respondent_office}</TableCell><TableCell className="max-w-48 truncate" title={response.respondent_name}>{response.respondent_name}</TableCell><TableCell><span className="text-base font-black">{response.score}</span></TableCell><TableCell><ClassificationBadge value={response.classification} /></TableCell><TableCell className="max-w-xs truncate text-muted-foreground" title={response.score_reason || undefined}>{response.score_reason || "—"}</TableCell><TableCell><div className="flex">{canEdit && <Button variant="ghost" size="icon" className="h-8 w-8" aria-label="Editar resposta" onClick={() => openEdit(response)}><Pencil className="h-4 w-4" /></Button>}{canDelete && <Button variant="ghost" size="icon" className="h-8 w-8" aria-label="Excluir resposta" onClick={() => setDeleting(response)}><Trash2 className="h-4 w-4 text-destructive" /></Button>}</div></TableCell></TableRow>)}{!filtered.length && <TableRow><TableCell colSpan={7} className="h-24 text-center text-sm text-muted-foreground">Nenhuma resposta encontrada.</TableCell></TableRow>}</TableBody></Table><div className="px-3 pb-3"><NpsPaginationBar currentPage={currentResponsePage} pageSize={responsePageSize} totalItems={filtered.length} totalPages={responseTotalPages} itemLabel="respostas" selectLabel="Respostas por página" onPageChange={setResponsePage} onPageSizeChange={updateResponsePageSize} /></div></CardContent></Card></TabsContent><TabsContent value="history" className="mt-3"><Card><CardContent className="p-0"><Table className="[&_td]:px-3 [&_td]:py-2 [&_th]:h-9 [&_th]:px-3"><TableHeader><TableRow><TableHead>Cartório</TableHead><TableHead>Período</TableHead><TableHead>Respostas</TableHead><TableHead>Promotores</TableHead><TableHead>Neutros</TableHead><TableHead>Detratores</TableHead><TableHead>NPS</TableHead></TableRow></TableHeader><TableBody>{pagedHistory.map((item) => <TableRow key={item.id}><TableCell className="max-w-64 truncate font-medium" title={item.registry_office?.name}>{item.registry_office?.name}</TableCell><TableCell className="whitespace-nowrap text-xs">{formatDateOnly(item.period_start)} – {formatDateOnly(item.period_end)}</TableCell><TableCell>{item.total_responses}</TableCell><TableCell>{item.total_promoters}</TableCell><TableCell>{item.total_neutrals}</TableCell><TableCell>{item.total_detractors}</TableCell><TableCell className="font-black">{item.nps_score.toFixed(1)}</TableCell></TableRow>)}{!history.length && <TableRow><TableCell colSpan={7} className="h-24 text-center text-sm text-muted-foreground">Nenhum histórico encontrado.</TableCell></TableRow>}</TableBody></Table><div className="px-3 pb-3"><NpsPaginationBar currentPage={currentHistoryPage} pageSize={historyPageSize} totalItems={history.length} totalPages={historyTotalPages} itemLabel="períodos" selectLabel="Períodos por página" onPageChange={setHistoryPage} onPageSizeChange={updateHistoryPageSize} /></div></CardContent></Card></TabsContent></Tabs>
    ...
    <Dialog open={importOpen} onOpenChange={setImportOpen}><DialogContent><DialogHeader><DialogTitle>Importar respostas NPS</DialogTitle><DialogDescription>Use XLSX ou CSV com Data/Hora, Nome e Nota/Pontuação. Motivo e Sugestão são opcionais; duplicatas serão ignoradas.</DialogDescription></DialogHeader><div className="space-y-4"><Field label="Cartório de destino"><Select value={importOfficeId} onValueChange={setImportOfficeId}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent>{offices.map((office) => <SelectItem key={office.id} value={office.id}>{office.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Arquivo XLSX ou CSV"><Input type="file" accept=".xlsx,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv" onChange={(event) => setImportFile(event.target.files?.[0] ?? null)} /></Field></div><DialogFooter><Button variant="outline" onClick={() => setImportOpen(false)}>Cancelar</Button><Button disabled={!importOfficeId || !importFile || importResponses.isPending} onClick={handleImport}>Importar</Button></DialogFooter></DialogContent></Dialog>
...
function Metric({ icon: Icon, label, value }: { icon: typeof Star; label: string; value: number }) { return <Card><CardContent className="flex items-center gap-2.5 px-3 py-2.5"><div className="rounded-lg bg-rose-50 p-1.5 text-rose-600 dark:bg-rose-950/40"><Icon className="h-4 w-4" /></div><div><p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{label}</p><p className="text-xl font-black leading-6">{value}</p></div></CardContent></Card>; }
function NpsPaginationBar({ currentPage, pageSize, totalItems, totalPages, itemLabel, selectLabel, onPageChange, onPageSizeChange }: { currentPage: number; pageSize: number; totalItems: number; totalPages: number; itemLabel: string; selectLabel: string; onPageChange: (page: number) => void; onPageSizeChange: (size: string) => void }) {
