...
export default function CsCxRoutines() {
  ...
  const totals = useMemo(() => {
    const analyzed = officeSummaries.filter(
                      ...
                      <TableRow key={summary.registryOfficeId}>
                        ...
                        <TableCell className="px-3 py-2">
                          ...
                          >
                            ...
                            {summary.analyzed ? "Analisado" : "Não analisado"}
                          </Badge>
      ...
      >
        <DialogContent className="flex max-h-[94vh] flex-col overflow-hidden sm:max-w-6xl">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            {openedOfficeRoutines.length > 0 &&
              openedOfficeRoutines.every((routine) =>
                canEditRecord(routine.applied_by),
              ) && (
              <Button type="button" className="h-9" onClick={openBulkAnalysis}>
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={closeOfficeAnalysis}>
              Fechar
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="space-y-4">
            ...
            <ObservationListEditor
              ...
              addLabel="Adicionar observação"
              description="As observações serão registradas em todos os itens do cartório."
            />
          </div>
          <DialogFooter>
            <Button
...
function AnalysisMetric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-lg border bg-muted/20 px-3 py-2">
      ...
      <p className="text-lg font-black leading-6">{value}</p>
    </div>
  );
}
...
function RoutineItemRow({
  ...
  return (
    <div className="flex flex-col justify-between gap-2 rounded-md border px-3 py-2 md:flex-row md:items-center">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 truncate text-[11px] leading-4 text-muted-foreground">
          ...
          {beforeCount || afterCount
            ? ` · Observações: ${beforeCount} antes / ${afterCount} depois`
            : ""}
        </p>
      </div>
      <div className="flex flex-wrap items-center gap-1.5">
