...
export default function CsCxAdmin() {
  ...
  if (admin.isLoading) {
    return <div className="container mx-auto max-w-7xl space-y-4 p-6"><Skeleton className="h-24 w-full" /><Skeleton className="h-96 w-full" /></div>;
  }
  ...
  return (
    <div className="container mx-auto max-w-7xl space-y-6 p-4 md:p-6">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          <div className="flex items-center gap-2 text-sm font-medium text-rose-600 dark:text-rose-300"><Settings2 className="h-4 w-4" />CS/CX</div>
          <h1 className="mt-1 text-3xl font-black tracking-tight">Administração de rotinas</h1>
          <p className="text-sm text-muted-foreground">Gerencie modelos, itens, categorias, tipos e produtos vinculados.</p>
        </div>
        {!canManage && <Badge variant="outline">Somente leitura</Badge>}
      </div>

      {admin.error && <Card className="border-destructive/40"><CardContent className="flex items-center justify-between gap-4 pt-6"><div className="flex items-center gap-2 text-sm text-destructive"><TriangleAlert className="h-4 w-4" />{messageOf(admin.error)}</div><Button variant="outline" size="sm" onClick={() => admin.refetch()}><RefreshCw className="mr-2 h-4 w-4" />Tentar novamente</Button></CardContent></Card>}
      ...
      <Tabs defaultValue="models" className="space-y-4">
        <TabsList><TabsTrigger value="models">Modelos e itens</TabsTrigger><TabsTrigger value="categories">Categorias</TabsTrigger><TabsTrigger value="types">Tipos</TabsTrigger></TabsList>

        <TabsContent value="models" className="grid gap-4 lg:grid-cols-[300px_1fr]">
          <Card>
            <CardHeader className="pb-3"><div className="flex items-center justify-between"><div><CardTitle className="text-base">Modelos</CardTitle><CardDescription>Selecione para administrar os itens.</CardDescription></div>{canManage && <Button size="icon" variant="outline" aria-label="Criar modelo" onClick={() => openModel()}><Plus className="h-4 w-4" /></Button>}</div></CardHeader>
            <CardContent className="space-y-2">
              {admin.models.map((model) => <button key={model.id} type="button" onClick={() => setSelectedModelId(model.id)} className={`w-full rounded-lg border p-3 text-left transition-colors ${selectedModelId === model.id ? "border-rose-300 bg-rose-50/60 dark:border-rose-900 dark:bg-rose-950/20" : "hover:bg-muted/50"}`}><div className="flex items-start justify-between gap-2"><span className="text-sm font-semibold">{model.name}</span><Badge variant={model.active ? "default" : "secondary"}>{model.active ? "Ativo" : "Inativo"}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{model.item_count} itens · {model.products.length} produtos</p></button>)}
              {!admin.models.length && <p className="py-8 text-center text-sm text-muted-foreground">Nenhum modelo cadastrado.</p>}
            </CardContent>
          </Card>

          <Card>
            {selectedModel ? <>
              <CardHeader className="pb-3"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><CardTitle>{selectedModel.name}</CardTitle>{selectedModel.origin === "legacy" && <Badge variant="outline">Legado</Badge>}</div><CardDescription className="mt-1">{selectedModel.description || "Sem descrição"}</CardDescription><div className="mt-2 flex flex-wrap gap-1">{selectedModel.products.map((product) => <Badge key={product.id} variant="secondary">{product.name}</Badge>)}</div></div>{canManage && <div className="flex gap-1"><Button variant="outline" size="sm" onClick={() => openModel(selectedModel)}><Pencil className="mr-2 h-3.5 w-3.5" />Editar</Button><Button variant="ghost" size="icon" aria-label="Excluir modelo" onClick={() => setDeleting({ kind: "model", id: selectedModel.id, name: selectedModel.name })}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>}</div></CardHeader>
              <CardContent className="space-y-3 border-t pt-4">
                <div className="flex items-center justify-between"><div><h2 className="font-semibold">Itens do modelo</h2><p className="text-xs text-muted-foreground">A ordem abaixo é usada nas aplicações dos cartórios.</p></div>{canManage && <Button size="sm" onClick={() => openItem()}><Plus className="mr-2 h-4 w-4" />Novo item</Button>}</div>
                {selectedItems.map((item, index) => <div key={item.id} className="flex flex-col justify-between gap-3 rounded-lg border p-3 sm:flex-row sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">{item.sort_order}. {item.name}</span>{item.required && <Badge variant="secondary">Obrigatório</Badge>}<Badge variant="outline" style={{ borderColor: item.category?.display_color }}>{item.category?.name ?? "Sem categoria"}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{item.routine_type?.name ?? "Sem tipo"} · padrão: {statusLabel(item.default_active)}</p></div>{canManage && <div className="flex shrink-0 gap-1"><Button variant="ghost" size="icon" aria-label={`Subir ${item.name}`} disabled={index === 0 || admin.reorderItem.isPending} onClick={() => moveItem(item, -1)}><ArrowUp className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Descer ${item.name}`} disabled={index === selectedItems.length - 1 || admin.reorderItem.isPending} onClick={() => moveItem(item, 1)}><ArrowDown className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Editar ${item.name}`} onClick={() => openItem(item)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Excluir ${item.name}`} onClick={() => setDeleting({ kind: "item", id: item.id, name: item.name })}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>}</div>)}
                {!selectedItems.length && <div className="rounded-lg border border-dashed py-10 text-center text-sm text-muted-foreground">Este modelo ainda não possui itens.</div>}
              </CardContent>
            </> : <CardContent className="py-20 text-center text-sm text-muted-foreground">Selecione ou crie um modelo.</CardContent>}
          </Card>
        </TabsContent>
...
function CatalogPanel({ kind, items, canManage, onCreate, onEdit, onDelete }: {
  ...
  return <Card><CardHeader><div className="flex items-center justify-between"><div><CardTitle>{label}s de rotina</CardTitle><CardDescription>Cadastros usados para classificar os itens dos modelos.</CardDescription></div>{canManage && <Button onClick={onCreate}><Plus className="mr-2 h-4 w-4" />Novo {label.toLocaleLowerCase("pt-BR")}</Button>}</div></CardHeader><CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <div key={item.id} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2">{kind === "category" && "display_color" in item && <span className="h-3 w-3 rounded-full" style={{ backgroundColor: item.display_color }} />}<h3 className="font-semibold">{item.name}</h3></div><p className="mt-1 text-xs text-muted-foreground">{item.description || "Sem descrição"}</p></div><Badge variant={item.active ? "default" : "secondary"}>{item.active ? "Ativo" : "Inativo"}</Badge></div><div className="mt-4 flex items-center justify-between"><span className="text-xs text-muted-foreground">{item.item_count} itens vinculados</span>{canManage && <div className="flex gap-1"><Button variant="ghost" size="icon" aria-label={`Editar ${item.name}`} onClick={() => onEdit(item)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Excluir ${item.name}`} disabled={item.item_count > 0} onClick={() => onDelete(item)}><Trash2 className="h-4 w-4 text-destructive" /></Button></div>}</div></div>)}{!items.length && <div className="col-span-full py-12 text-center text-sm text-muted-foreground">Nenhum cadastro encontrado.</div>}</CardContent></Card>;
}
