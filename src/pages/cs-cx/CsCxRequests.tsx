      ...
      <Card>
        <CardContent className="space-y-3 p-3">
          <div className="grid gap-2 lg:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_160px_200px_360px]">
            ...
            <div className="flex h-9 min-w-0 items-center gap-1 rounded-md border bg-background px-2" title="Período da solicitação"><CalendarClock aria-hidden="true" className="h-4 w-4 shrink-0 text-muted-foreground" /><span className="sr-only">Período da solicitação</span><Input aria-label="Período inicial da solicitação" type="date" value={periodStart} max={periodEnd || undefined} onChange={(event) => updatePeriodStart(event.target.value)} className="h-7 w-[132px] min-w-[132px] border-0 px-1 text-xs shadow-none focus-visible:ring-0" /><span className="shrink-0 text-[11px] text-muted-foreground">até</span><Input aria-label="Período final da solicitação" type="date" value={periodEnd} min={periodStart || undefined} onChange={(event) => updatePeriodEnd(event.target.value)} className="h-7 w-[132px] min-w-[132px] border-0 px-1 text-xs shadow-none focus-visible:ring-0" />{(periodStart || periodEnd) && <Button type="button" variant="ghost" size="icon" className="h-6 w-6 shrink-0" aria-label="Limpar período" onClick={() => { setPeriodStart(""); setPeriodEnd(""); setPage(1); }}><X className="h-3.5 w-3.5" /></Button>}</div>
          </div>
