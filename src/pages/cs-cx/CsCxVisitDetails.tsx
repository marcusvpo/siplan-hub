...
export default function CsCxVisitDetails({ visit, canEdit, canDelete }: Props) {
  ...
  return <div className="grid gap-6 lg:grid-cols-2">
    ...
    <section className="lg:col-span-2">
      ...
      {canEdit && <div className="mb-3 grid min-w-0 gap-2 md:grid-cols-[1fr_1fr_auto]"><Input className="min-w-0" ref={fileRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.gif,.doc,.docx,.xls,.xlsx,.txt" /><Input className="min-w-0" value={attachmentDescription} onChange={(event) => setAttachmentDescription(event.target.value)} placeholder="Descrição opcional" /><Button className="w-full md:w-auto" disabled={uploadAttachment.isPending} onClick={() => uploadFile(fileRef.current?.files?.[0])}>Enviar</Button></div>}
