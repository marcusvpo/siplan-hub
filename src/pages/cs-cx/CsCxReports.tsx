...
export default function CsCxReports() {
  ...
  return (
    <div data-testid="cs-cx-reports-page" className="container mx-auto flex w-full min-w-0 max-w-[1600px] flex-col space-y-4 overflow-x-hidden px-3 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] max-md:[&>div:last-child]:hidden sm:px-4 lg:px-6">
      ...
      <div className="grid grid-cols-2 gap-2 xl:grid-cols-5 [&>*:last-child]:col-span-2 xl:[&>*:last-child]:col-span-1"><Metric icon={ListChecks} label="Aplicações" value={analytics.totalApplications} /><Metric icon={CheckCircle2} label="Aplicações ativas" value={analytics.activeApplications} /><Metric icon={CheckCircle2} label="Itens ativos" value={analytics.activeItems} /><Metric icon={ListChecks} label="Itens inativos" value={analytics.inactiveItems} /><Metric icon={TriangleAlert} label="A analisar" value={analytics.pendingItems} /></div>
