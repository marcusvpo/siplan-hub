...
function AppointmentTable({ appointments, canEdit, canDelete, onEdit, onAction, onDelete }: { appointments: CsCxAppointment[]; canEdit: boolean; canDelete: boolean; onEdit: (item: CsCxAppointment) => void; onAction: (item: CsCxAppointment, status: string) => void; onDelete: (item: CsCxAppointment) => void }) {
  return <div className="overflow-x-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead className="h-9 px-3 text-xs">Data</TableHead><TableHead className="h-9 px-3 text-xs">Compromisso</TableHead><TableHead className="h-9 px-3 text-xs">Cartório/Contato</TableHead><TableHead className="h-9 px-3 text-xs">Responsável</TableHead><TableHead className="h-9 px-3 text-xs">Tipo</TableHead><TableHead className="h-9 px-3 text-xs">Status</TableHead><TableHead className="h-9 w-12 px-2" /></TableRow></TableHeader><TableBody>{appointments.length === 0 ? <TableRow><TableCell colSpan={7} className="h-24 text-center text-muted-foreground">Nenhum agendamento encontrado.</TableCell></TableRow> : appointments.map((item) => <TableRow key={item.id}><TableCell className="whitespace-nowrap px-3 py-2"><div className="text-sm font-medium">{formatDateTime(item.starts_at)}</div><div className="text-[11px] leading-4 text-muted-foreground">{item.duration_minutes} min</div></TableCell><TableCell className="px-3 py-2"><div className="max-w-64 truncate text-sm font-medium" title={item.title}>{item.title}</div><div className="max-w-64 truncate text-[11px] leading-4 text-muted-foreground" title={item.location || item.description || "Sem detalhes"}>{item.location || item.description || "Sem detalhes"}</div></TableCell><TableCell className="px-3 py-2"><div className="max-w-52 truncate text-sm" title={item.registry_office?.name}>{item.registry_office?.name || "—"}</div><div className="max-w-52 truncate text-[11px] leading-4 text-muted-foreground" title={item.contact?.contact_person}>{item.contact?.contact_person || "Sem contato vinculado"}</div></TableCell><TableCell className="max-w-44 truncate px-3 py-2 text-sm" title={item.responsible?.full_name || item.responsible?.email || "Não vinculado"}>{item.responsible?.full_name || item.responsible?.email || "Não vinculado"}</TableCell><TableCell className="px-3 py-2"><Badge variant="secondary" className="h-5 px-1.5 text-[10px] font-normal">{TYPE_LABELS[item.appointment_type] ?? item.appointment_type}</Badge></TableCell><TableCell className="px-3 py-2"><StatusBadge appointment={item} /></TableCell><TableCell className="px-2 py-1">{(canEdit || canDelete) && <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8"><MoreHorizontal className="h-4 w-4" /><span className="sr-only">Ações</span></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{canEdit && <><DropdownMenuItem onClick={() => onEdit(item)}><Pencil className="mr-2 h-4 w-4" />Editar</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => onAction(item, "REALIZADO")}>Marcar realizado</DropdownMenuItem><DropdownMenuItem onClick={() => onAction(item, "CONCLUIDO")}>Concluir</DropdownMenuItem><DropdownMenuItem onClick={() => onAction(item, "REMARCADO")}>Remarcar</DropdownMenuItem><DropdownMenuItem onClick={() => onAction(item, "CANCELADO")} className="text-amber-700">Cancelar</DropdownMenuItem></>}{canDelete && <><DropdownMenuSeparator /><DropdownMenuItem onClick={() => onDelete(item)} className="text-destructive"><Trash2 className="mr-2 h-4 w-4" />Excluir</DropdownMenuItem></>}</DropdownMenuContent></DropdownMenu>}</TableCell></TableRow>)}</TableBody></Table></div>;
}

function AppointmentPaginationBar({ currentPage, pageSize, totalItems, totalPages, onPageChange, onPageSizeChange }: { currentPage: number; pageSize: number; totalItems: number; totalPages: number; onPageChange: (page: number) => void; onPageSizeChange: (size: string) => void }) {
  const firstItem = totalItems === 0 ? 0 : (currentPage - 1) * pageSize + 1;
  const lastItem = Math.min(currentPage * pageSize, totalItems);
  return <div className="flex flex-col gap-2 border-t pt-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between"><span aria-label={`Mostrando ${firstItem} a ${lastItem} de ${totalItems} agendamentos`}>Mostrando <strong className="font-semibold text-foreground">{firstItem}–{lastItem}</strong> de <strong className="font-semibold text-foreground">{totalItems}</strong></span><div className="flex flex-wrap items-center gap-2"><span>Por página</span><Select value={String(pageSize)} onValueChange={onPageSizeChange}><SelectTrigger aria-label="Agendamentos por página" className="h-8 w-[72px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="5">5</SelectItem><SelectItem value="10">10</SelectItem><SelectItem value="20">20</SelectItem><SelectItem value="50">50</SelectItem></SelectContent></Select><span className="min-w-[92px] text-center">Página {currentPage} de {totalPages}</span><Button type="button" variant="outline" size="icon" className="h-8 w-8" aria-label="Página anterior" disabled={currentPage <= 1} onClick={() => onPageChange(currentPage - 1)}><ChevronLeft className="h-4 w-4" /></Button><Button type="button" variant="outline" size="icon" className="h-8 w-8" aria-label="Próxima página" disabled={currentPage >= totalPages} onClick={() => onPageChange(currentPage + 1)}><ChevronRight className="h-4 w-4" /></Button></div></div>;
}
...
function MonthCalendar({ month, onMonthChange, appointments, onEdit }: { month: string; onMonthChange: (value: string) => void; appointments: CsCxAppointment[]; onEdit?: (item: CsCxAppointment) => void }) {
  ...
  return <div className="space-y-2"><div className="flex justify-end"><Input aria-label="Mês do calendário" type="month" value={month} onChange={(event) => onMonthChange(event.target.value)} className="h-9 w-44" /></div><div className="grid grid-cols-7 overflow-hidden rounded-lg border">{["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"].map((day) => <div key={day} className="border-b bg-muted/40 p-1.5 text-center text-[11px] font-semibold">{day}</div>)}{slots.map((day, index) => {
    ...
    return <div key={`${day ?? "empty"}-${index}`} className={cn("min-h-20 border-b border-r p-1", !day && "bg-muted/20")}><span className="text-[11px] text-muted-foreground">{day}</span><div className="mt-0.5 space-y-0.5">{items.slice(0, 3).map((item) => <button key={item.id} type="button" onClick={() => onEdit?.(item)} className="w-full truncate rounded bg-rose-50 px-1 py-0.5 text-left text-[10px] text-rose-800 hover:bg-rose-100 dark:bg-rose-950/40 dark:text-rose-200">{new Date(item.starts_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })} {item.title}</button>)}{items.length > 3 && <p className="text-[10px] text-muted-foreground">+{items.length - 3} itens</p>}</div></div>;
  })}</div></div>;
}

function StatusBadge({ appointment }: { appointment: CsCxAppointment }) { const overdue = appointment.status === "AGENDADO" && new Date(appointment.starts_at) < new Date(); return <Badge variant="outline" className={cn("h-5 px-1.5 text-[10px] font-normal", overdue && "border-red-200 bg-red-50 text-red-700", ["REALIZADO", "CONCLUIDO"].includes(appointment.status) && "border-emerald-200 bg-emerald-50 text-emerald-700", appointment.status === "CANCELADO" && "border-slate-200 bg-slate-100 text-slate-600", appointment.status === "REMARCADO" && "border-amber-200 bg-amber-50 text-amber-700")}>{overdue ? "Vencido" : STATUS_LABELS[appointment.status] ?? appointment.status}</Badge>; }
function Metric({ label, value, icon: Icon }: { label: string; value: number; icon: typeof CalendarDays }) { return <Card><CardContent className="flex items-center justify-between px-3 py-2.5"><div><p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{label}</p><p className="text-xl font-bold leading-6">{value}</p></div><Icon className="h-4 w-4 text-rose-500" /></CardContent></Card>; }
